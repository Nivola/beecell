...
def usage(func):
    ...
    def inner(*args, **kwargs):
        ...
        items = {
            'rss': {'desc': 'Resident Set Size - this is the non-swapped physical memory a process has used',
                    'value': rss},
            'vms': {'desc': 'Virtual Memory Size - this is the total amount of virtual memory used by the process',
                    'value': vms},
            'shared': {'desc': 'Memory that could be potentially shared with other processes', 'value': shared},
            'text': {'desc': 'TRS (text resident set) the amount of memory devoted to executable code', 'value': text},
            'data': {'desc': 'DRS (data resident set) the amount of physical memory devoted to other than executable '
                             'code', 'value': data},
...
def watch(func):
    ...
    def inner(*args, **kwargs): #1
        ...
        info = '%s:%s - %s - START -  %s:%s' % (cp.ident, ct.ident, task_id,
                                                func.__module__ + '.' + get_member_class(args), func.__name__)
        # logger.info(info)
        ...
        info = '%s:%s - %s - STOP  -  %s:%s - %s' % (cp.ident, ct.ident, task_id, func.__module__ + '.' +
                                                     get_member_class(args), func.__name__, elapsed)

...
if __name__ == "__main__":
    ...
    if len(sys.argv) is 1:
        print 'Usage: python fileserver.py path=/usr/local/files host=127.0.0.1 port=8080 mountpoint=/files'
        sys.exit
    ...
    application = WSGIFileServerApplication(root_path=args['path'], mount_point=args['mountpoint'])
   
    server = simple_server.make_server(args['host'], int(args['port']), application)
   
    try:
        if args['mountpoint'] is None: args['mountpoint'] = ''
        print 'Bring up portal2 at http://%s:%s%s' % (args['host'], args['port'], args['mountpoint'])
        while 1:

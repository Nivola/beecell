...
def check_tax_code(codice_fiscale):
    ...
    if (16 != len(codice_fiscale)):
        logger.error("La lunghezza del codice fiscale non è corretta: il codice fiscale dovrebbe essere lungo esattamente 16 caratteri.")
        return False

    codice_fiscale = codice_fiscale.upper()
    match = re.match(CODICE_REGEXP, codice_fiscale)
    if not match:
        logger.error("Il codice fiscale contiene dei caratteri non validi: i soli caratteri validi sono le lettere e le cifre.")
        return False
